...
with 
	...
	best_ss_customer as(
		...
		group by 
			c_customer_sk
		having 
			sum(ss_quantity*ss_sales_price) > (95/100.0) * (select tpcds_cmax from max_store_sales limit 1))
select  
	sum(sales)
from 
	(select 
	...
	where 
		...
		and ws_bill_customer_sk in (select c_customer_sk from best_ss_customer)) as dt
limit 100;
