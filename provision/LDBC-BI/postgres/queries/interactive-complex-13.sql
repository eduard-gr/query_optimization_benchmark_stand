WITH RECURSIVE search_graph(link, depth, path) AS (
        SELECT 8796093022390, 0, ARRAY[8796093022390]::bigint[]
      UNION ALL
          ...
          and not exists(select * from sg y where y.link = 8796093022357)
...
select depth from search_graph where link = 8796093022357
union select -1) tmp;
